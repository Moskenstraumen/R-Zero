...
def sglang_generate_batch(session, solver_url, prompts, sampling_params, request_timeout):
    payload = {"text": prompts, "sampling_params": sampling_params}
    res = session.post(solver_url, json=payload, timeout=request_timeout)
...
def _parse_args():
    ...
    parser.add_argument(
        "--question-batch-size",
        dest="question_batch_size",
        type=int,
        default=DEFAULT_QUESTION_BATCH_SIZE,
    )
...
def _build_result_entry(question, outputs):
...
def main():
    ...
    question_batch_size = max(args.question_batch_size, 1)
    print(
        f"[{args.suffix}] question_batch_size={question_batch_size} "
        f"(effective_prompt_batch={question_batch_size * args.num_samples})"
    )
    ...
    with requests.Session() as session:
        for batch_start in range(0, total_questions, question_batch_size):
            ...
            for prompt in batch_prompts:
                expanded_prompts.extend([prompt] * args.num_samples)
            ...
            for offset, question in enumerate(batch_questions):
                ...
                output_start = offset * args.num_samples
                output_end = output_start + args.num_samples
                question_outputs = outputs[output_start:output_end]

                try:
                    result_entry = _build_result_entry(question, question_outputs)
